...
string g_upf_s11_ip_addr = "10.129.26.169";
string g_upf_s1_ip_addr = "10.129.26.169";
int g_upf_s11_port = 7000;
int g_upf_s1_port = 7100;
string g_upf_sgi_ip_addr = "10.129.26.169";
string g_sink_ip_addr = "10.129.26.169";
int g_upf_sgi_port = 8100;
int g_sink_port = 8500;
int smf_s11_port=8000;
//upf is same as sgw+pgw and smf is mme's part
UeContext::UeContext() {
	tai = 0; 
	apn_in_use = 0; 
	eps_bearer_id = 0;
...
void Upf::handle_create_session(struct sockaddr_in src_sock_addr, Packet pkt, UdpClient &pgw_s5_client){
...
void Upf::handle_uplink_udata(Packet pkt, UdpClient &s11_cteid_smf) {
	uint64_t imsi;
	uint32_t s11_uteid_ul;
	string smf_s11_ip_addr;
	int smf_s11_port;
	bool res;
	...
	res = get_uplink_info(imsi, s11_uteid_ul, smf_s11_ip_addr, smf_s11_port);
	if (res) {
		pkt.truncate();
		pkt.prepend_gtp_hdr(1, 2, pkt.len, s11_uteid_ul);
		s11_cteid_smf.set_server(smf_s11_ip_addr, smf_s11_port);
		s11_cteid_smf.snd(pkt);		
...
void Upf::handle_detach(struct sockaddr_in src_sock_addr, Packet pkt, UdpClient &pgw_s5_client) {
	uint64_t imsi;
	uint64_t tai;
	uint32_t s1_uteid_ul;
	...
	s11_cteid_smf = ue_ctx[imsi].s11_cteid_smf;
	s11_cteid_upf = ue_ctx[imsi].s11_cteid_upf;
	s1_uteid_ul = ue_ctx[imsi].s1_uteid_ul;
	//s5_uteid_dl = ue_ctx[imsi].s5_uteid_dl;	
	g_sync.munlock(uectx_mux);
	pgw_s5_client.set_server(g_upf_s11_ip_addr, g_upf_s11_port);
	pkt.clear_pkt();
	pkt.append_item(eps_bearer_id);
	pkt.append_item(tai);
	pkt.prepend_gtp_hdr(2, 4, pkt.len, s11_cteid_smf);
	...
	pkt.clear_pkt();
	pkt.append_item(res);
	pkt.prepend_gtp_hdr(2, 3, pkt.len, s11_cteid_smf);
	s11_server.snd(src_sock_addr, pkt);
	TRACE(cout << "sgw_handledetach:" << " detach response sent to mme: " << imsi << endl;)

	rem_itfid(11, s11_cteid_upf);
...
bool Upf::get_uplink_info(uint64_t imsi, uint32_t &s11_cteid_ul, string &s11_cteid_smf, int &smf_s11_port) {
	bool res = false;

	g_sync.mlock(uectx_mux);
	if (ue_ctx.find(imsi) != ue_ctx.end()) {
		res = true;
//		s11_cteid_ul = ue_ctx[imsi].s11_cteid_ul;
		s11_cteid_smf = ue_ctx[imsi].s11_cteid_smf;
//		smf_s11_port = ue_ctx[imsi].smf_s11_port;
	}
	g_sync.munlock(uectx_mux);
	return res;
}
